...
def test_parameter_reset():
    """Test resetting module parameters"""
    rw_power = ReactionWheelPower(
        base_power_need=5.0,
        elec_to_mech_eff=0.8,
        mech_to_elec_eff=0.6
    ...
    # Reset parameters
    rw_power.reset_parameters(
        base_power_need=3.0,
        elec_to_mech_eff=0.9,
        mech_to_elec_eff=0.7
    )
    
    # Check new values
    assert torch.isclose(rw_power.base_power_need, torch.tensor(3.0))
    assert torch.isclose(rw_power.elec_to_mech_eff, torch.tensor(0.9))
    assert torch.isclose(rw_power.mech_to_elec_eff, torch.tensor(0.7))
    
    # Test partial reset
    rw_power.reset_parameters(base_power_need=4.0)
    assert torch.isclose(rw_power.base_power_need, torch.tensor(4.0))
    assert torch.isclose(rw_power.elec_to_mech_eff, torch.tensor(0.9))  # Unchanged
