...
class ReactionWheelPower(nn.Module):
    ...
    def __init__(self, 
        ...
        if elec_to_mech_eff <= 0.0:
            raise ValueError("elec_to_mech_eff must be a strictly positive value.")
        
        # 注册为不可训练参数
        self.register_buffer("base_power_need", torch.tensor(base_power_need))
        self.register_buffer("elec_to_mech_eff", torch.tensor(elec_to_mech_eff))
        self.register_buffer("mech_to_elec_eff", torch.tensor(mech_to_elec_eff))
