...
class ReactionWheelPower(Module[ReactionWheelPowerStateDict]):
    ...
    def reset(self) -> ReactionWheelPowerStateDict:
        ...
        state_dict.update(
            base_power_need = torch.tensor(0.0, dtype=torch.float32),
            elec_to_mech_eff = torch.tensor(1.0, dtype=torch.float32),
            mech_to_elec_eff = torch.tensor(-1.0, dtype=torch.float32),
        )
        return state_dict
    ...
    def forward(
        ...
        **kwargs, 
        ...
        accel_power = self._base_power_need + torch.abs(wheel_power) / self._elec_to_mech_effi
        regen_power = self._base_power_need + self._mech_to_elec_efficiency * wheel_power
